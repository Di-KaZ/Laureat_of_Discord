class Player:
    def __init__(self, user, categories, num_round):
        ...
        self.board = [['' for x in range(len(categories) + 2)] for y in range(int(num_round) + 1)]
        for i, category in enumerate(categories):
            self.board[0][i + 1] = category
        self.board[0][0] = user.name + '#' + user.discriminator
        for i in range(1, num_round + 1):
            self.board[i][len(categories) + 1] = '0'
    ...
    def roundStart(self, letter, num_round):
        self.board[num_round + 1][0] = letter

    def addWord(self, num_category, word, actual_round):
        self.board[actual_round + 1][int(num_category)] = word
        return
    
    def finishRound(self, actual_round):
    ...
    def addScoreToRound(self, points, actual_round):
        ...
        self.board[actual_round][len(self.categories) + 1] = prev_round_score + points 
    ...
    def getScore(self):
        ...
        for i in range(1, self.num_round):
            score += int(self.board[i][len(self.categories) + 1])
        return score
...
class PlayersManager:
    ...
    def roundStart(self, players, letter, actual_round):
        for player in players:
            player.roundStart(letter, actual_round)
    
    async def isPlayersRoundFinish(self, players, actual_round):
        for player in players:
            if player.finishRound(actual_round):
                for player in players:
                    await player.getUser().send(f"Le round {actual_round + 1} est terminé {player.getUser().mention} est le premier a l'avoir terminé !")
                return True
        return False
